...
mod tests {
    ...
    fn test_cap_manifest_creation() {
        let id = CapCard::from_string("cap:action=extract;target=metadata;").unwrap();
    ...
    fn test_cap_manifest_with_multiple_caps() {
        let id1 = CapCard::from_string("cap:action=extract;target=metadata;").unwrap();
        let cap1 = Cap::new(id1, "1.0.0".to_string(), "extract-metadata".to_string());
        
        let id2 = CapCard::from_string("cap:action=extract;target=outline;").unwrap();
        ...
        assert_eq!(manifest.caps[0].id_string(), "cap:action=extract;target=metadata");
        assert_eq!(manifest.caps[1].id_string(), "cap:action=extract;target=outline");
    ...
    fn test_cap_manifest_optional_author_field() {
        let id = CapCard::from_string("cap:action=validate;type=file").unwrap();
    ...
    fn test_component_metadata_trait() {
        ...
        let id = CapCard::from_string("cap:action=test;type=component").unwrap();
        ...
        assert_eq!(caps[0].id_string(), "cap:action=test;type=component");
    }
}